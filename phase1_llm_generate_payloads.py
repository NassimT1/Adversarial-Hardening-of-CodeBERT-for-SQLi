...
class SQLiAgentGenerator:
    ...
    def __init__(
        ...
        # Clean up empty strings that might come from .env
        if gemini_api_key and gemini_api_key.strip() in ["", '""', "''"]:
            gemini_api_key = None
    ...
    def batch_generate(
        ...
        for category in ATTACK_CATEGORIES:
            for context in SQL_TEMPLATES:
                ...
                with open("data/generated_payloads.json", "w") as f:
                    json.dump(all_records, f, indent=2)
                pd.DataFrame(all_records).to_csv("data/generated_payloads.csv", index=False)
...
df_out.to_csv("data/generated_payloads.csv", index=False)
